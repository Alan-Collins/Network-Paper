...
ggplot(melt.dat.thresh.removed, aes(x=Var1, y=value, col="grey80")) +
  geom_point(data = melt.dat.thresh.removed, aes(x = Var1, y=value, colour="grey80"), position = position_jitterdodge(dodge.width = 1), alpha = 1, size = 0.5) +
  geom_point(data = melt.dat.thresh, aes(x = Var1, y=value, colour="red"), position = position_jitterdodge(dodge.width = 1, jitter.width = 0.7), alpha = 1, size = 1) +
  #geom_text_repel(data = nanostring.phenotypes, aes(x = nanostring.phenotypes$Gene, y = nanostring.phenotypes$Expression, label = nanostring.phenotypes$Label),
  #             size = 2, force =0.25, min.segment.length = unit(0, "lines"))
  ggtitle("Expression relative to mean base medium") + xlab("Gene") + ylab("Log2 ratio expression") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line.x = element_line(colour = "black", size =0.5), axis.line.y = element_line(colour = "black", size =0.5)) +
  theme(axis.text.x = element_text(angle=90, hjust = 1, vjust = 0.5, size = 15), axis.text.y = element_text(size = 15), text = element_text(size = 20), legend.text = element_text(size = 12)) +
  scale_x_discrete("Gene", labels= mylabels, limits = order)+
  theme(legend.position="none") +
  geom_hline(yintercept = 0) +
  ggsave(filename = "Results/scatterplot gene expression thresholded.pdf", width = 15, height = 5, units = "in")

  
ggplot(melt.dat.thresh.removed, aes(x=Var2, y=value, col="grey80")) +
  geom_point(data = melt.dat.thresh.removed, aes(x = Var2, y=value, color="grey80"), position = position_jitterdodge(dodge.width = 1), size = 0.5) +
  geom_point(data = melt.dat.thresh, aes(x = Var2, y=value, color="red"), position = position_jitterdodge(dodge.width = 1, jitter.width = 0.7), size = 1) +
  # geom_text_repel(data = nanostring.phenotypes, aes(x = nanostring.phenotypes$Gene, y = nanostring.phenotypes$Expression ), label = nanostring.phenotypes$Label,
  #                size = 2, force =0.25, min.segment.length = unit(0, "lines"))+
  ggtitle("Expression relative to mean base medium") + xlab("Condition") + ylab("Log2 ratio expression") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  ...
  theme(legend.position="none") +
  geom_hline(yintercept = 0) +
  ggsave(filename = "Results/scatterplot gene expression thresholded by CS.pdf", width = 15, height = 5, units = "in")

nanostring.phenotypes <- nanostring.phenotypes[-no.change.rows, 1:3]

nanostring.phenotypes<- nanostring.phenotypes[order(nanostring.phenotypes$Condition),]

write.csv(nanostring.phenotypes, file = "Results/Nanostring.phenotypes.csv")

colors = c(seq(min(dat.thresh),-0.5,length=20),seq(-0.49,0.49,length=2),seq(0.5,max(dat.thresh),length=20)) #Define colour range

my_palette <- colorRampPalette(c("Blue","black", "red"))(n=15)

pdf(paste("Results/nanostring_thresh_log2.pdf", sep=""), width=15, height=20)
heatmap.2(dat.thresh, col=my_palette, margins=c(10,10), cexRow = 1, cexCol = 1,
          srtCol = 60, trace = "none", key = T, density.info = "none")
dev.off()

uni.melt <- melt(as.matrix(log2(sweep(uni, 1, apply(uni,1,mean),  FUN = "/"))))

ggplot(melt.dat.unthresh, aes(x=Var1, y=value, col="black")) +
  geom_point(data = melt.dat.unthresh, aes(x=Var1, y=value, col="black"),position = position_jitterdodge(dodge.width = 1), size = 0.5, alpha = 1) +
  geom_point(data = uni.melt, aes(x=Var1, y=value, col="blue"),position = position_jitterdodge(dodge.width = 1), size = 1, alpha = 1) +
  # geom_text_repel(data = nanostring.phenotypes, aes(x = nanostring.phenotypes$Gene, y = nanostring.phenotypes$Expression ), label = nanostring.phenotypes$Label,
  #                size = 2, force =0.25, min.segment.length = unit(0, "lines"))+
  ggtitle("Expression relative to mean base medium") + xlab("Condition") + ylab("Log2 ratio expression") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  ...
  theme(legend.position="none") +
  geom_hline(yintercept = 0) +
  ggsave(filename = "Results/scatterplot gene expression base medium overlay.pdf", width = 15, height = 5, units = "in")
